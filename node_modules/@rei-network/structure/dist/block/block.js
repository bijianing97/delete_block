...
const common_1 = require("@rei-network/common");
...
class Block {
    ...
    static fromBlockData(blockData = {}, opts) {
        ...
        for (const uhData of uhsData !== null && uhsData !== void 0 ? uhsData : []) {
            const uh = header_1.BlockHeader.fromHeaderData(uhData, {
    ...
    static fromValuesArray(values, opts) {
        ...
        for (const uncleHeaderData of uhsData || []) {
            uncleHeaders.push(header_1.BlockHeader.fromValuesArray(uncleHeaderData, {
                hardforkByBlockNumber: true,
                ...opts,
                // Use header common in case of hardforkByBlockNumber being activated
                common: header._common,
     ...
     */
    raw() {
        return [this.header.raw(), this.transactions.map((tx) => (tx.supports(tx_1.Capability.EIP2718TypedTransaction) ? tx.serialize() : tx.raw())), this.uncleHeaders.map((uh) => uh.raw())];
    }
    /**
    ...
    get size() {
        let size = 0;
        for (const buf of this.header.raw()) {
            size += buf.length;
        }
    ...
    /**
     * Convert block to rpc json
     * @param isPending - Target block is pending or not
     * @param fullTransactions - Return full transactions or only hashes
     */
    toRPCJSON(isPending, fullTransactions) {
        return {
            number: isPending ? null : (0, ethereumjs_util_1.bnToHex)(this.header.number),
            hash: isPending ? null : (0, ethereumjs_util_1.bufferToHex)(this.hash()),
            parentHash: (0, ethereumjs_util_1.bufferToHex)(this.header.parentHash),
            nonce: isPending ? null : (0, ethereumjs_util_1.bufferToHex)(this.header.nonce),
            sha3Uncles: (0, ethereumjs_util_1.bufferToHex)(this.header.uncleHash),
            logsBloom: isPending ? null : (0, ethereumjs_util_1.bufferToHex)(this.header.bloom),
            transactionsRoot: (0, ethereumjs_util_1.bufferToHex)(this.header.transactionsTrie),
            stateRoot: (0, ethereumjs_util_1.bufferToHex)(this.header.stateRoot),
            receiptsRoot: (0, ethereumjs_util_1.bufferToHex)(this.header.receiptTrie),
            miner: this.header.coinbase.toString(),
            mixHash: (0, ethereumjs_util_1.bufferToHex)(this.header.mixHash),
            difficulty: (0, ethereumjs_util_1.bnToHex)(this.header.difficulty),
            totalDifficulty: (0, ethereumjs_util_1.bnToHex)(this.header.number),
            extraData: (0, ethereumjs_util_1.bufferToHex)(this.header.extraData),
            size: (0, ethereumjs_util_1.intToHex)(this.size),
            gasLimit: (0, ethereumjs_util_1.bnToHex)(this.header.gasLimit),
            gasUsed: (0, ethereumjs_util_1.bnToHex)(this.header.gasUsed),
            timestamp: (0, ethereumjs_util_1.bnToHex)(this.header.timestamp),
            transactions: fullTransactions
                ? this.transactions.map((tx, i) => {
                    if (tx.extension === undefined) {
                        tx.initExtension(this);
                    }
                    return tx.toRPCJSON();
                })
                : this.transactions.map((tx) => (0, ethereumjs_util_1.bufferToHex)(tx.hash())),
            uncles: this.uncleHeaders.map((uh) => (0, ethereumjs_util_1.bufferToHex)(uh.hash()))
        };
    }
}
exports.Block = Block;
